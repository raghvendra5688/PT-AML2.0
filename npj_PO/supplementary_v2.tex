...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.92\textwidth]{jtrm_images/Supp_Fig1.pdf}
...
%% Fig S2 ─ Patients per inhibitor
\begin{figure}[!ht]
    \centering
    \subfloat[Fig S2A][Distribution of AUC drug response values across training (Waves~1+2) and test (Waves~3+4)
    patient samples. No statistically significant difference was observed ($p > 0.05$, Wilcoxon rank-sum test),
    confirming comparable drug sensitivity distributions between splits.]{
        \includegraphics[width=0.72\textwidth]{figures/Train_Test_Y_distribution.pdf}
    }
    \qquad
    \subfloat[Fig S2B][Number of patient samples tested per inhibitor in the BeatAML dataset. Drugs vary
    substantially in the number of assayed patients; inhibitors tested on fewer patients contribute to
    wider prediction confidence intervals in the TabPFN ensemble.]{
        \includegraphics[width=0.72\textwidth]{figures/patients_per_inhibitor.pdf}
    }
    \caption{\textbf{Drug response distribution and per-inhibitor data coverage statistics for the BeatAML cohort.}}
    \label{fig:suppfig2}
\end{figure}
...
\toprule
\textbf{Model} & \textbf{Class} & \textbf{CV MAE} & \textbf{Test MAE} & \textbf{Test RMSE} & \textbf{Test $r_{pc}$} & \textbf{Test R$^2$}\\
\midrule
...
& & \textbf{MAE} & $r_{pc}$ & \textbf{MAE} & $r_{pc}$ \\
\midrule
