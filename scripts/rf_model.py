...
os.makedirs("../Results/rf/optuna/", exist_ok=True)
...
def hyperparameter_optimization(X_train,Y_train,data_type,stratify_by,scaler):
    ...
    def objective(trial):
        params = {
            ...
            'max_features': trial.suggest_float('max_features',0.02,0.3),
        ...
        model = ensemble.RandomForestRegressor(**params, bootstrap=True, oob_score=False, n_jobs=-1, verbose=1)
    ...
    study.trials_dataframe().to_csv(f"logs/optuna_trials_rf_{data_type}_{stratify_by}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)

    #Save the model
    best_model = ensemble.RandomForestRegressor(**study.best_params, n_jobs=-1, bootstrap=True, oob_score=False, verbose=1)
    best_model.fit(X_train, Y_train)

    # Save the best model
    save_model(best_model, "%s_models/%s_%s_%s_optuna_best.pk" %("rf","rf", data_type, stratify_by))
            
    ##Save the scaler 
    #save_model(scaler, "%s_models/%s_%s_%s_scaling_gs.pk" % ("rf","rf",data_type,stratify_by))
...
def make_predictions(best_model, X_test, Y_test, data_type, stratify_by):
    ...
    metadata_X_test.to_csv(f"../Results/rf/optuna/{data_type}_{stratify_by}_optuna_predictions_drug.csv", sep="\t", index=False)
    print(f"Finished writing predictions for optuna optimized RF model with data type: {data_type} and stratified by:{stratify_by}.")
...
def make_prediction_plot(metadata_X_test,Y_test,y_pred, test_metrics, data_type, stratify_by):
    ...
    ax = sn.regplot(x="labels", y="predictions", data=metadata_X_test, scatter_kws={"color": "yellow",'alpha':0.5}, 
                line_kws={"color": "red"})
    title = "RF Prediction "+data_type+"_"+stratify_by
    ...
    outfilename = "../Results/rf/optuna/"+data_type+"_"+stratify_by+"_supervised_test_prediction.pdf"
...
def make_importance_plot(best_model, X_train, data_type, stratify_by):
    ...
    title = "Top RF Coefficient for "+data_type+"_"+stratify_by
    ax.axes.set_title(title,fontsize=10)
    ax.set_xlabel("Features",fontsize=10)
    ax.set_ylabel("Coefficient Value",fontsize=10)
    ax.tick_params(labelsize=8)
    outputfile = "../Results/rf/optuna/"+data_type+"_"+stratify_by+"_Coefficients.pdf"
...
stratify_options = ["inhibitor","dbgap_rnaseq_sample"]
...
for input_option in range(0,len(data_type_options)):
    ...
    for stratify_by in stratify_options:
        ...
        #Load best model
        model_path = "../Models/rf_models/rf_"+data_type+"_"+stratify_by+"_optuna_best.pk"
