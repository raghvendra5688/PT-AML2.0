...
RESULTS_DIR = "../Results/tabpfn_fimmaml/optuna"
PLOT_DIR    = "../Results/tabpfn_fimmaml/plots"
...
POINT_COLOR  = "#2166AC"   # blue
LINE_COLOR   = "#D73027"   # red regression line
ALPHA        = 0.25
POINT_SIZE   = 8


def scatter_one(ax, x, y, title, xlabel="DSS", ylabel="Predicted AUC"):
    ...
    # Tight axis limits with 3 % padding
...
for fpath in pred_files:
    data_type = os.path.basename(fpath).replace("_optuna_predictions.csv", "")
    title     = LABEL_MAP.get(data_type, data_type)

    df = pd.read_csv(fpath, sep="\t")
    df = df.dropna(subset=["predicted_auc", "DSS"])
    x  = df["DSS"].to_numpy(dtype=float)
    y  = df["predicted_auc"].to_numpy(dtype=float)

    fig, ax = plt.subplots(figsize=(4.5, 4.5))
    pearson_r, spearman_r = scatter_one(ax, x, y, title)
    fig.tight_layout(pad=1.0)

    out_path = os.path.join(PLOT_DIR, f"{data_type}_scatter.pdf")
...
for idx, fpath in enumerate(pred_files):
    ...
    df = pd.read_csv(fpath, sep="\t").dropna(subset=["predicted_auc", "DSS"])
    x  = df["DSS"].to_numpy(dtype=float)
    y  = df["predicted_auc"].to_numpy(dtype=float)
...
fig.suptitle("FIMM-AML: Predicted AUC vs DSS (BeatAML-trained model)",
             fontsize=13, fontweight="bold", y=1.01)
fig.tight_layout(pad=1.5)

combined_path = os.path.join(PLOT_DIR, "FIMMAML_all_configs_scatter.pdf")
...
summary_path = os.path.join(RESULTS_DIR, "all_data_types_test_metrics_summary.csv")
