...
from dl_model_architecture import (NN_Encoder, GATNet, GATNet_Enhanced, Seq2Func_Net, Seq2Func_Net_Enhanced,
...
os.makedirs("../Results/gat/optuna/", exist_ok=True)
os.makedirs("../Models/gat_models/", exist_ok=True)
...
# Function to get GPU with most free memory
def get_best_gpu():
...
cudaid = get_best_gpu() if torch.cuda.is_available() else None
if cudaid is not None:
    DEVICE = torch.device("cuda:%d" % cudaid)
    print(f"Using GPU {cudaid} with {torch.cuda.mem_get_info(cudaid)[0] / 1024**3:.2f} GB free memory")
else:
...
CLIP = 1.0
OUT_DIM = 1
SMILES_INPUT_DIM = 78  # Atom feature dimension for GAT
...
def define_model(CELL_INPUT_DIM, cell_out_dim, cell_hid_dims, dropout, SMILES_INPUT_DIM, smiles_n_heads,
                 smiles_hid_dim, smiles_out_dim, fusion_hid_dim, OUT_DIM, init_weights):

    # -------- Model --------
    cell_enc = NN_Encoder(CELL_INPUT_DIM, cell_out_dim, cell_hid_dims, dropout)

    smiles_enc = GATNet(SMILES_INPUT_DIM, smiles_n_heads, smiles_hid_dim, smiles_out_dim, dropout)
...
def final_model_training(model, train_loader, valid_loader, optimizer, criterion, N_dim, data_type, stratify_by):
    print(f"Performing final GAT model training for {data_type} using {stratify_by}")
    train_loss_list = []
    valid_loss_list = []
    best_valid_loss = float('inf')
    patience_ctr = 0
    outputfile_model = f"../Models/gat_models/gat_optuna_{data_type}_{stratify_by}.pt"
    for epoch in range(N_EPOCHS):
        if (patience_ctr<PATIENCE):
    ...
    # Load best model with explicit GPU memory cleanup
    if (torch.cuda.is_available()):
        # Clear current model from GPU before loading
        model.cpu()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    fout_filename = "../Models/gat_models/gat_"+data_type+"_"+stratify_by+"_loss_plot.csv"
...
def objective(trial, full_train_dataset, metadata_X_train, stratify_by, CELL_INPUT_DIM, SMILES_INPUT_DIM):
    ...
    cell_out_dim = trial.suggest_int("cell_out_dim", 128, 256, step=64)
    n_cell_layers = trial.suggest_int("n_cell_layers", 1, 3)
    cell_hid_dims = [
        trial.suggest_int(f"cell_hid_{i}", 128, 256, step=64)
        for i in range(n_cell_layers)
    ]

    # GAT parameters (simple - fixed 3 GAT layers)
    smiles_n_heads = trial.suggest_int("smiles_n_heads", 2, 4, step=2)
    smiles_hid_dim = trial.suggest_int("smiles_hid_dim", 64, 256, step=64)
    smiles_out_dim = trial.suggest_int("smiles_out_dim", 64, 256, step=64)

    # Fusion network parameters (single hidden dim)
    ...
    if groups is not None:
        for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(full_train_dataset)), groups=groups)):
            ...
            model = define_model(CELL_INPUT_DIM, cell_out_dim, cell_hid_dims, dropout, SMILES_INPUT_DIM,
                                smiles_n_heads, smiles_hid_dim, smiles_out_dim, fusion_hid_dim, OUT_DIM,
                                init_weights)
...
def run_pipeline(train_dataset, test_dataset, metadata_X_train, stratify_by, CELL_INPUT_DIM, SMILES_INPUT_DIM, data_type, scaler):
    ...
    study.trials_dataframe().to_csv(f"logs/optuna_trials_gat_{data_type}_{stratify_by}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)
    ...
    smiles_enc = GATNet(SMILES_INPUT_DIM, p["smiles_n_heads"], p["smiles_hid_dim"], p["smiles_out_dim"], p["dropout"])
    ...
    ##Save the scaler
    save_model(scaler, "%s_models/%s_%s_%s_scaling_gs.pk" % ("gat","gat",data_type,stratify_by))
...
def make_predictions(y_pred, Y_test, data_type, stratify_by):
    ...
    metadata_X_test.to_csv(f"../Results/gat/optuna/{data_type}_{stratify_by}_optuna_predictions_drug.csv", sep="\t", index=False)
    print(f"Finished writing predictions for optuna optimized GAT model with data type: {data_type} and stratified by:{stratify_by}.")
    return test_metrics
...
def make_prediction_plot(metadata_X_test, Y_test, y_pred, test_metrics, data_type, stratify_by):
    ...
    ax = sn.regplot(x="labels", y="predictions", data=metadata_X_test, scatter_kws={"color": "yellow",'alpha':0.5},
                line_kws={"color": "red"})
    title = "GAT Prediction "+data_type+"_"+stratify_by
    ...
    outfilename = "../Results/gat/optuna/"+data_type+"_"+stratify_by+"_supervised_test_prediction.pdf"
