...
import math
import datetime
from dl_model_architecture import (NN_Encoder, CNN_Encoder, Seq2Func, init_weights, count_parameters, training, evaluation, epoch_time, evaluation_performance)


os.makedirs("logs", exist_ok=True)
os.makedirs("../Results/cnn/", exist_ok=True)
os.makedirs("../Results/cnn/optuna/", exist_ok=True)
...
def define_model(CELL_INPUT_DIM, cell_out_dim, cell_hid_dims, dropout, SMILES_INPUT_DIM, smiles_emb_dim,smiles_out_dim,n_filters,filter_sizes,hid_dim,OUT_DIM,init_weights):

    # -------- Model --------
    cell_enc = NN_Encoder(CELL_INPUT_DIM,cell_out_dim,cell_hid_dims,dropout)

    smiles_enc = CNN_Encoder(SMILES_INPUT_DIM,smiles_emb_dim,smiles_out_dim,n_filters,filter_sizes,dropout)
...
def final_model_training(model, train_loader, valid_loader, optimizer, criterion, data_type, stratify_by):
    print(f"Performing final CNN model training for {data_type} using {stratify_by}")
    train_loss_list = []
    valid_loss_list = []
    best_valid_loss = float('inf')
    patience_ctr = 0
    outputfile_model = f"../Models/cnn_models/cnn_optuna_{data_type}_{stratify_by}.pt"
    for epoch in range(N_EPOCHS):
        if (patience_ctr<PATIENCE):
    ...
    fout_filename = "../Models/cnn_models/cnn_"+data_type+"_"+stratify_by+"_loss_plot.csv"
    ...
    if groups is not None:
        for fold, (train_idx, val_idx) in enumerate(kf.split(full_train_dataset, groups=groups)):
            ...
            model = define_model(CELL_INPUT_DIM, cell_out_dim, cell_hid_dims, dropout, SMILES_INPUT_DIM, smiles_emb_dim,
                                smiles_out_dim,n_filters,filter_sizes,hid_dim,OUT_DIM,init_weights)
...
def run_pipeline(train_dataset, test_dataset, metadata_X_train, stratify_by, CELL_INPUT_DIM, SMILES_INPUT_DIM, data_type, scaler):
    ...
    # study.trials_dataframe().to_csv(f"logs/optuna_trials_cnn_{data_type}_{stratify_by}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)
    ...
    # # =============================
    # p = study.best_params
    if data_type=="Only_PC_Feat_Var":
        p={'cell_out_dim': 512, 'cell_hid_1': 768, 'cell_hid_2': 1024, 'smiles_emb_dim': 64, 'smiles_out_dim': 128, 'n_filters': 16, 'hid_dim': 384, 'dropout': 0.19756042908403829, 'lr': 0.0009027510074098364, 'weight_decay': 1.1174598119937625e-06}
    elif data_type=="Embed_Feat_Var":
        p={'cell_out_dim': 512, 'cell_hid_1': 768, 'cell_hid_2': 1024, 'smiles_emb_dim': 64, 'smiles_out_dim': 128, 'n_filters': 16, 'hid_dim': 384, 'dropout': 0.19441059627495783, 'lr': 0.0009388108773044956, 'weight_decay': 1.4001099612154864e-06}
    elif data_type=="ChemBERTa_Feat_Var":
        p={'cell_out_dim': 512, 'cell_hid_1': 768, 'cell_hid_2': 1024, 'smiles_emb_dim': 64, 'smiles_out_dim': 128, 'n_filters': 16, 'hid_dim': 384, 'dropout': 0.19441059627495783, 'lr': 0.0009388108773044956, 'weight_decay': 1.4001099612154864e-06}
    elif data_type=="MolFormer_Feat_Var":
        p={'cell_out_dim': 512, 'cell_hid_1': 768, 'cell_hid_2': 1024, 'smiles_emb_dim': 64, 'smiles_out_dim': 128, 'n_filters': 16, 'hid_dim': 384, 'dropout': 0.19441059627495783, 'lr': 0.0009388108773044956, 'weight_decay': 1.4001099612154864e-06}
        
    #Define model
    cell_enc = NN_Encoder(CELL_INPUT_DIM, p["cell_out_dim"], [p["cell_hid_1"], p["cell_hid_2"]], p["dropout"],)
    smiles_enc = CNN_Encoder(SMILES_INPUT_DIM, p["smiles_emb_dim"], p["smiles_out_dim"], p["n_filters"], filter_sizes, p["dropout"])
    ...
    ##Save the scaler 
    save_model(scaler, "%s_models/%s_%s_%s_scaling_gs.pk" % ("cnn","cnn",data_type,stratify_by))
...
def make_predictions(y_pred, Y_test, data_type, stratify_by):
    ...
    metadata_X_test.to_csv(f"../Results/cnn/optuna/{data_type}_{stratify_by}_optuna_predictions_drug.csv", sep="\t", index=False)
    print(f"Finished writing predictions for optuna optimized CNN model with data type: {data_type} and stratified by:{stratify_by}.")
    return test_metrics
...
def make_prediction_plot(metadata_X_test,Y_test, y_pred, test_metrics, data_type, stratify_by):
    ...
    ax = sn.regplot(x="labels", y="predictions", data=metadata_X_test, scatter_kws={"color": "yellow",'alpha':0.5}, 
                line_kws={"color": "red"})
    title = "CNN Prediction "+data_type+"_"+stratify_by
    ...
    outfilename = "../Results/cnn/optuna/"+data_type+"_"+stratify_by+"_supervised_test_prediction.pdf"
