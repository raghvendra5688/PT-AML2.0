...
# %% [markdown]
# # TabPFN Ablation Study - 1-Group Combinations
#
# Runs all single patient-group experiments. Each is run with Drug_PC, Drug_Embed, and Drug_PC+Drug_Embed.
...
# %% [markdown]
# ## Run 1-Group Combinations

# %%
group_names = sorted(ablation_groups.keys())
k = 1
combo_list = list(combinations(group_names, k))
print(f"Running {len(combo_list)} {k}-group combinations\n")

for combo in combon_list:
    combo_name = "+".join(combo)

    for drug_name, drug_cols in drug_feature_options.items():
        experiment_name = f"combo_{k}_{combo_name}_{drug_name}"

        feature_subset = list(drug_cols)
        for g in combo:
            feature_subset.extend(ablation_groups[g])
...
# %%
with open("../Results/tabpfn/ablation/ablation_results_1group.pkl", "wb") as f:
    pickle.dump(ablation_results, f)
print(f"Saved {len(ablation_results)} results to ablation_results_1group.pkl")
