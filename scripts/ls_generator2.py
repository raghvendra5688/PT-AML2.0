...
def run_smiles_generator(test_file):
    ...
    train_data,valid_data = train.split(split_ratio=0.9, random_state=random.seed(SEED))
    ...
    model.load_state_dict(torch.load('../Models/torchtext_checkpoint.pt',map_location=torch.device('cpu')))
...
if __name__=='__main__':
    ...
    input_df = pd.read_csv("../Data/SMILES_Autoencoder/"+args.input,header='infer')
    all_sequences = input_df['src'].values.tolist()
    ...
    #Write output 
    ls_df.to_csv("../Data/SMILES_Autoencoder/"+args.output,index=False)
