...
def hyperparameter_optimization(X_train,Y_train,data_type,stratify_by):
    ...
    def objective(trial):
        ...
        #model = LSSVMRegressor(**params)
        model = NeoLSSVM(**params)
        ...
        if groups is not None:
            ...
            scores = cross_val_score(estimator=model, X=X_train, y=Y_train, groups=groups, scoring="neg_mean_absolute_error",cv=cv, n_jobs=10)
            return np.mean(scores)
        else:
            cv = KFold(n_splits=5, shuffle=True, random_state=42)
            scores = cross_val_score(model, X_train, Y_train, scoring="neg_mean_absolute_error", cv=cv, n_jobs=10)
            return np.mean(scores)
    ...
    study.trials_dataframe().to_csv(f"logs/optuna_trials_neosvr_{data_type}_{stratify_by}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)

    #Save the model
    #best_model = LSSVMRegressor(**study.best_params)
    best_model = NeoLSSVM(**study.best_params)
    best_model.fit(X_train, Y_train)

    # Save the best model
    save_model(best_model, "%s_models/%s_%s_%s_optuna_best.pk" %("neosvr","neosvr", data_type, stratify_by))
            
    #Save the scaler 
    save_model(scaler, "%s_models/%s_%s_%s_scaling_gs.pk" % ("neosvr","neosvr",data_type,stratify_by))
...
def make_predictions(best_model, X_test, Y_test, data_type, stratify_by):
    ...
    metadata_X_test.to_csv(f"../Results/neosvr/optuna/{data_type}_{stratify_by}_optuna_predictions_drug.csv", sep="\t", index=False)
    print(f"Finished writing predictions for optuna optimized SVR model with data type: {data_type} and stratified by:{stratify_by}.")
...
def make_prediction_plot(metadata_X_test,Y_test,y_pred, test_metrics, data_type, stratify_by):
    ...
    ax = sn.regplot(x="labels", y="predictions", data=metadata_X_test, scatter_kws={"color": "yellow",'alpha':0.5}, 
                line_kws={"color": "red"})
    title = "NeoSVR Preiction "+data_type+"_"+stratify_by
    ...
    outfilename = "../Results/neosvr/optuna/"+data_type+"_"+stratify_by+"_supervised_test_prediction.pdf"
...
def make_importance_plot(best_model, X_train, data_type, stratify_by):
    ...
    title = "Top NeoSVR Coefficient for "+data_type+"_"+stratify_by
    ax.axes.set_title(title,fontsize=10)
    ax.set_xlabel("Features",fontsize=10)
    ax.set_ylabel("Coefficient Value",fontsize=10)
    ax.tick_params(labelsize=8)
    outputfile = "../Results/neosvr/optuna/"+data_type+"_"+stratify_by+"_Coefficients.pdf"
...
data_type_options = ["Only_PC_Feat_Var","Embed_Feat_Var","MolFormer_Feat_Var","ChemBERTa_Feat_Var"]
...
for input_option in range(0,len(data_type_options)):
    ...
    for stratify_by in stratify_options:
        ...
        #Load best model
        model_path = "../Models/neosvr_models/neosvr_"+data_type+"_"+stratify_by+"_optuna_best.pk"
