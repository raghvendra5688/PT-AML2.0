...
os.makedirs("../Results/glr/", exist_ok=True)
os.makedirs("../Results/glr/optuna/", exist_ok=True)
...
def hyperparameter_optimization(X_train,Y_train,data_type,stratify_by):

    def objective(trial):
        params = {
            'alpha': trial.suggest_float("alpha",1,1e4,log=True),
        ...
        if groups is not None:
            ...
            #     scores.append(metrics.mean_absolute_error(Y_train[valid_idx], y_pred))
            # return np.mean(scores)
            scores = cross_val_score(estimator=model,X=X_train, y=Y_train, groups=groups, scoring="neg_mean_absolute_error", cv = cv, n_jobs=60)
            return np.mean(scores)
        else:
            cv = KFold(n_splits=5, shuffle=True, random_state=42)
            scores = cross_val_score(model, X_train, Y_train, scoring="neg_mean_absolute_error", cv=cv, n_jobs=60)
            return np.mean(scores)
    ...
    study.trials_dataframe().to_csv(f"logs/optuna_trials_glr_{data_type}_{stratify_by}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)

    #Save the model
    best_model = linear_model.Ridge(**study.best_params)
    best_model.fit(X_train, Y_train)

    # Save the best model
    save_model(best_model, "%s_models/%s_%s_%s_optuna_best.pk" %("glr","glr", data_type, stratify_by))
            
    #Save the scaler 
    save_model(scaler, "%s_models/%s_%s_%s_scaling_gs.pk" % ("glr","glr",data_type,stratify_by))
...
def make_predictions(best_model, X_test, Y_test, data_type, stratify_by):
    ...
    metadata_X_test.to_csv(f"../Results/glr/optuna/{data_type}_{stratify_by}_optuna_predictions_drug.csv", sep="\t", index=False)
    print(f"Finished writing predictions for optuna optimized GLR model with data type: {data_type} and stratified by:{stratify_by}.")
...
def make_prediction_plot(metadata_X_test,Y_test,y_pred, test_metrics, data_type, stratify_by):
    ...
    ax = sn.regplot(x="labels", y="predictions", data=metadata_X_test, scatter_kws={"color": "yellow",'alpha':0.5}, 
                line_kws={"color": "red"})
    title = "GLR Predictions "+data_type+"_"+stratify_by
    ...
    outfilename = "../Results/glr/optuna/"+data_type+"_"+stratify_by+"_supervised_test_prediction.pdf"
...
def make_importance_plot(best_model, X_train, data_type, stratify_by):
    val, index = np.sort(np.abs(best_model.coef_)), np.argsort(np.abs(best_model.coef_))
    ...
    title = "Top GLR Coefficients "+data_type+"_"+stratify_by
    ax.axes.set_title(title,fontsize=10)
    ax.set_xlabel("Features",fontsize=10)
    ax.set_ylabel("Coefficient Value",fontsize=8)
    ax.tick_params(labelsize=8)
    outputfile = "../Results/glr/optuna/"+data_type+"_"+stratify_by+"_Coefficients.pdf"
    plt.savefig(outputfile, bbox_inches="tight")


#Get the setting with different X_trains and X_tests
train_options = ["../Data/Training_Set_Var_with_Drug_Embedding_Patient_Info.pkl",
...
test_options = ["../Data/Test_Set_Var_with_Drug_Embedding_Patient_Info.pkl",
...
data_type_options = ["Embed_Feat_Var","MolFormer_Feat_Var","ChemBERTa_Feat_Var","Only_PC_Feat_Var"]
stratify_options = ["inhibitor","dbgap_rnaseq_sample","random"]

# +
# #Choose the options
# input_option, stratify_option = 0,0                                                  #Choose 0 for Embed, 1 for MFP, 2 for ChemBERTa, 3 for All
# data_type = data_type_options[input_option]
# stratify_by = stratify_options[stratify_option]
...
for input_option in range(0,len(data_type_options)):
    ...
    for stratify_by in stratify_options:
        ...
        hyperparameter_optimization(X_train,Y_train,data_type,stratify_by)

        #Load best model
        model_path = "../Models/glr_models/glr_"+data_type+"_"+stratify_by+"_optuna_best.pk"
